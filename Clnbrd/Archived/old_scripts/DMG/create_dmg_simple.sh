...
# Clnbrd Simple DMG Creator Script
# Creates a basic DMG installer with drag-to-Applications interface
...
echo -e "${YELLOW}📦 Creating temporary DMG (50MB)...${NC}"
hdiutil create -srcfolder "${APP_PATH}" -volname "${VOLUME_NAME}" -fs HFS+ -fsargs "-c c=64,a=16,e=16" -format UDRW -size 50m "${DMG_TEMP_NAME}.dmg"
